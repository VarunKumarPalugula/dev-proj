...
SVGElement.prototype.toDataURL = function(type, options) {
	...
	function exportImage(type) {
		...
		svg_img.onload = function() {
			...
			debug(type + " lengthhh: " + png_dataurl.length);
	...
	function exportImageCanvg(type) {
		var canvas = document.createElement("canvas");
			//canvas.attr("fill", 'rgba(0,0,0,0.5)');
		var ctx = canvas.getContext('2d');
			/*ctx.clearRect (0, 0, _svg.clientWidth, _svg.clientHeight);
			ctx.fillStyle = 'rgba(0,0,0,0.5)';
			ctx.shadowColor = 'rgba(0,0,0,0.5)';
			ctx.fillRect(0, 0, _svg.clientWidth, _svg.clientHeight);
		...
		canvg(canvas, svg_xml, { 
			...
			renderCallback: function() {
				...
				var png_dataurl = canvas.toDataURL(type);
				debug(type + " exportImageCanvg length: " + png_dataurl.length);
        ...
        var dataTmp = ctx.getImageData(0,0, canvas.width, canvas.height);		
        //store the current globalCompositeOperation
        var compositeOperation = ctx.globalCompositeOperation;
        //set to draw behind current content
        ctx.globalCompositeOperation = "destination-over";
        //set background color
        ctx.fillStyle = '#2e7294'; //'rgba(0,255,0,1)';
		ctx.fillRect(0,0,canvas.width,canvas.height);
